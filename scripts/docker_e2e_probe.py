...
def main():
    ...
    result=detail.get("result") or {}; checks={"health":True,"registration":True,"rq_completed":detail["status"]=="completed","deterministic_result":bool(result.get("deterministic_results")),"product_intelligence":bool(result.get("product_intelligence")),"tenant_isolation":isolated,"bounded_duration":elapsed <= args.max_seconds}
